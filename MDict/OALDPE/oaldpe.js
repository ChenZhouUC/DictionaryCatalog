...
// “oaldpeconfig”、“oaldpecfg”、“oaldcfg”、“opcfg”、“oaldconfig”、“opconfig”、“opc”、“oaldpe”、“牛津配置”、“牛津设置”、“配置”和“设置”。
...
var oaldpeCfg = {
    ...
    // 选项（默认为2）：0-不启用官方例句发音，1-启用官方在线例句发音，2-启用官方离线例句发音.ogg 3-启用官方离线例句发音.mp3（需要去网盘“文件替换”文件夹里下载oaldpe.3.mdd替换掉原来的文件）4.使用TTS发音代替官方例句发音（需要配置项7启用时才生效）
...
(function ($) {
    ...
    function termNumberClickEvent() {
        $(".oaldpe .li_sense_before, .oaldpe .top-container").click(function(e) {
            e.stopPropagation(); 
    ...
    function fnOfficialExPronOpt(itemValue) {
        ...
        // 使用TTS发音代替官方例句发音（需要配置项7启用时才生效）
        (itemValue === 4) && oaldpeCfg.enableOnlineTTS && $("example-audio")
        .replaceWith('<example-audio-ai><a class="audio_uk" href="##"></a><a class="audio_us" href="##"></a></example-audio-ai>');
    ...
    function fnUnfoldSense(itemValue) {
        ...
        !itemValue && $(".oaldpe .li_sense_before").next().find("span.sensetop > div#ox-enlarge").hide();
    }
    ...
    function fnShowNavbar(itemValue) {
        ...
        // 牛津3000词等则不执行
        if ($('.oaldpe > .oald.extra > ul.top-g').length > 0)
            return;
    ...
    (async function initTTS() {
        ...
        !oaldpeCfg.enableOnlineTTS && $('.oaldpe example-audio-ai').addClass('audio_hide');    
        oaldpeCfg.enableOnlineTTS && $(".oaldpe example-audio-ai a.audio_uk, .oaldpe example-audio-ai a.audio_us").click(function(e){
            e.stopPropagation();
            e.preventDefault();
            ...
            eleExText = $(this).parent().siblings("div.exText, .en-link").clone();
            eleExText.find(".cf, chn, .zh").remove();
