...
> 验证方法：扫描全部按**扩展名**驱动（`find MDict -name '*.mdx/.css/.js'`）；对每个 `.css/.js` 用宽口径的"文件名 + 常见词典扩展名"通配正则提取候选引用，再用 `parent_dir/<basename>.exists()` 判断该引用是不是同目录下真实存在的磁盘兄弟。**不依赖**任何特定 stem 名（因为 stem 可能已被改过），任何"硬引用"都是经过该方法独立识别出来的。

跨文件引用扫描结果（本仓库 21 个 `.mdx`、10 个 `.css`、9 个 `.js`，加密体只读头部）：

- 21 个 `.mdx` 头部 `StyleSheet` 字段全部为空字串——`.css` 文件名不靠头部锚定，靠客户端"同主名兜底"
- 10 个 `.css` 里抽取出的非内联 / 非外网引用（`PrivateA.woff`、`cdoicons.woff`、`oalecd9.ttf`、`image/*.svg` 等）经 `parent/<basename>.exists()` 验证**没有任何一个**对应到同目录磁盘文件，全部解析进 `.mdd` 的虚拟资源包；CSS 对磁盘同级**零引用**
- 9 个 `.js` 里**只有一个**含磁盘同级硬引用：那个 `.js` 字面写了 3 个分卷 `.mdd` 文件名（对应 OALDPE 目录下的 3 个 `.mdd` 分卷），且作者用中文注释把每个分卷的功能都标了出来（在线发音 / 在线图片 / 离线例句发音）——属于**强硬编码**
- 另外两个 Perfect Edition `.js` 经同样口径的扩展名扫描没发现磁盘同级硬引用，但三个 `.js` 都通过 jQuery 操作 `.<stem>-config / .<stem>-nav / #<stem>-config` 这种 DOM 选择器；与之配套的 selector 数量经 `[.#]<stem>[a-z0-9_-]*` 字面计数为 OALDPE 40 / CDEPE 22 / MALDPE 14，`.mdx` HTML 正文里的 class 名也必然用同一套前缀（否则 .js 操作不到 DOM）
